...
sidebar_position: 2
description: "Why choose Offload Security over fragmented tools: one correlated source of truth that unifies cloud, code, container, Kubernetes, on-prem and endpoint findings into one governed risk view."
---
...
A detected issue flows into the **[Risk Register](../vulnerability-risk/risk-management/index.md)** with a treatment plan and SLA, updates the relevant **[compliance controls](../compliance/index.md)**, and attaches to **audit evidence** — turning day-to-day security work into continuously audit-ready governance.
...
Next, see the full module map in **[Platform at a Glance](./platform-at-a-glance.md)**, or explore the **[On-Premises & Private Infrastructure](../on-premises/index.mdx)** model.

<DocFaq items={[
  {
    q: "What makes Offload Security different from other security tools?",
    a: "Instead of adding another point tool, Offload consolidates cloud, code, container, Kubernetes, on-premises and Wazuh endpoint findings into one normalized model and a single governed risk view — and it runs as SaaS or fully on-premises.",
  },
  {
    q: "Is Offload Security just a CSPM?",
    a: "No. CSPM (cloud posture) is one module. Offload is a unified CNAPP, vulnerability management and compliance platform that also covers code, containers, Kubernetes, on-prem infrastructure, threat intelligence and GRC.",
  },
  {
    q: "How does Offload Security reduce security-tool sprawl?",
    a: "It runs the scanners itself — cloud, code, container, Kubernetes, web, API, network — and syncs Wazuh host findings into one vendor-neutral data lake, deduplicates them against a shared asset and finding model, and surfaces one risk-scored queue — so teams triage an issue once instead of reconciling a dozen consoles.",
  },
]} />
