...
The **[Vulnerability Dashboard](../vulnerability-risk/vulnerability-management.mdx)** is where the data lake becomes visible and actionable. It is not "the cloud scanner's results" or "the code scanner's results" — it is **every finding from every source, in one queue**, deduplicated, risk-scored, and tracked to closure:
