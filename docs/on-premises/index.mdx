...
sidebar_position: 0
description: "How Offload Security delivers CNAPP, vulnerability management and compliance on-premises — internal-network discovery, OpenVAS scanning and Wazuh telemetry — fully self-hosted, with data never leaving your environment."
---
...
![Offload Security internal infrastructure and endpoint visibility — private hosts, internal apps and Wazuh-monitored endpoints in one view](/img/screenshots/infra-endpoints.png)

:::tip[One platform, both worlds]
You don't run a separate on-prem product with its own console. Internal-network findings and Wazuh security events are correlated alongside cloud and application posture in one place — one inventory, one risk register, one compliance view — and a Greenbone/OpenVAS deployment connects in for internal vulnerability scanning.
:::
...
| **OpenVAS vulnerability scanning** | A supported integration for authenticated and unauthenticated vulnerability scanning of internal hosts and private environments; Offload connects to your Greenbone/OpenVAS deployment and OpenVAS performs the scanning. | [OpenVAS Scanning](./openvas-scanning.md) |
