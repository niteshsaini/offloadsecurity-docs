...
Every record on the platform is scoped to your **team**. All queries filter on that scope server-side — API keys and user sessions both resolve to a team before any data is read. Role-based access control ([Roles, Teams & API Keys](./authentication/rbac-team-management.md)) governs what members of a team can see and do, and an [audit trail](./authentication/audit-trail.md) records every authenticated request with actor, team, action and outcome, sensitive fields redacted, for 365 days by default.

## Credentials & secrets handling

Cloud credentials, registry credentials, integration tokens and Knowledge Base provider keys you store on the platform are **encrypted at rest** with authenticated symmetric encryption (Fernet: AES-128-CBC with HMAC-SHA256) under keys that live in the deployment's secret store or environment (`CLOUD_ENCRYPTION_KEY` for cloud credentials, `INTEGRATION_ENCRYPTION_KEY` for integration credentials) — generated by the setup wizard, never in application code. Secrets are decrypted only at the moment of use (when a scan authenticates to your cloud account, when an alert is posted to your webhook) and are never returned by the API once saved. User passwords are hashed (PBKDF2-SHA256, 600,000 iterations, per-user salt); session tokens, API keys and invitation tokens are stored only as SHA-256 hashes.
...
- **Authentication** — 24-hour user sessions (at most five per user, revocable by the platform administrator), optional TOTP MFA with deployment-wide enforcement, OIDC single sign-on, and scoped **API keys** for automation with expiry, IP allowlists, per-key rate limits and rotation. See [Platform Security](./authentication/index.md) and [API & Automation](./api-automation/index.md).
...
Retention is enforced platform-wide: scan results age out on schedules (`SCAN_RETENTION_DAYS` 90, cloud and container scans 180 by default), generated reports after 180 days (`REPORT_RETENTION_DAYS`), audit-trail events after 365 (`AUDIT_TRAIL_RETENTION_DAYS`), and regulated records where the regulation requires specific periods (for example, [DPDP audit records](./compliance/dpdp-privacy.md)), and deleting a team or account cascades through the platform's data stores rather than orphaning records. On request, customer data is deleted at contract end.
...
- [Platform Security](./authentication/index.md)
